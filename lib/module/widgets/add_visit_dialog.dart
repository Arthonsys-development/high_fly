...
class _AddVisitDialogState extends ConsumerState<AddVisitDialog>
  ...
  String _generateReraNumber() {
    ...
    // Create RERA number with format: RERA-{first4charsName}-{timestamp}
    final namePrefix = visitorName.length >= 4 
        ? visitorName.substring(0, 4).toUpperCase().replaceAll(RegExp(r'[^A-Z0-9]'), 'X')
        : visitorName.toUpperCase().padRight(4, 'X').replaceAll(RegExp(r'[^A-Z0-9]'), 'X');
    return 'RERA-$namePrefix-$timestamp';
  }
  ...
  Future<void> _saveVisit() async {
    ...
    if (isGuest == 'true') {
      debugPrint('Guest user attempting to save visit - showing alert');
      if (mounted) {
        _showGuestUserAlert();
      }
      return;
    ...
    showDialog(
      ...
      builder: (BuildContext dialogContext) {
        return AlertDialog(
          ...
          content: const Text(
            'Guest users cannot save visits. Please sign in with your phone number to access all features.',
            style: TextStyle(
              fontSize: 16,
          ...
          actions: [
            ...
            TextButton(
              onPressed: () async {
                ...
                try {
                  debugPrint('🚪 Logging out guest user from add visit dialog...');
                ...
                // Navigate to sign in screen using the captured widget context
                if (widgetContext.mounted) {
                  widgetContext.go(Routes.signIn);
                }
              },
              child: const Text(
                'Sign In',
                style: TextStyle(
                  color: Color.fromARGB(255, 0, 250, 4),
                  fontWeight: FontWeight.w600,
                  fontSize: 16,
                ),
              ),

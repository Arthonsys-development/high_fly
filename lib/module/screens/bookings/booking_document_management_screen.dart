...
import 'package:highfly/data/models/booking_document_model.dart' as booking_document_model;
...
class BookingDocumentManagementScreen extends ConsumerStatefulWidget {
  ...
  const BookingDocumentManagementScreen({super.key, required this.booking});

  @override
  ConsumerState<BookingDocumentManagementScreen> createState() => _BookingDocumentManagementScreenState();
}

class _BookingDocumentManagementScreenState extends ConsumerState<BookingDocumentManagementScreen> {
  final BookingApiRepository _repository = BookingApiRepository();
  final ImagePicker _imagePicker = ImagePicker();
  bool _isLoading = false;
  bool _isUploading = false;
  late List<booking_document_model.BookingDocument> _documents;
  ...
  String _detectFileType(String filePath) {
    ...
    } else if (fileName.contains('salary') || fileName.contains('slip')) {
      return 'salary_slip';
    } else if (fileName.contains('form') && fileName.contains('16')) {
      return 'form_16a';
    }
  ...
  void initState() {
    super.initState();
    _documents = List.from(widget.booking.documents);
  }
  ...
  Future<void> _refreshDocuments() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _documents = List.from(updatedBooking.documents);
          _isLoading = false;
        });
      }
    } catch (e) {
  ...
  Future<void> _pickImageFromCamera() async {
    try {
      debugPrint('📷 BookingDocumentManagement: Starting camera image pick');
      ...
      try {
        ...
        debugPrint('📷 BookingDocumentManagement: Image picker returned: ${pickedImage != null ? 'image captured' : 'no image'}');
      } catch (e) {
        debugPrint('❌ BookingDocumentManagement: Error opening camera: $e');
        ...
        if (errorString.contains('permission') || 
          ...
          if (!kIsWeb && mounted) {
            ...
            try {
              ...
              if (status.isPermanentlyDenied) {
                ...
                if (!status.isGranted) {
                  ...
                  try {
                    ...
                    debugPrint('❌ BookingDocumentManagement: Error on retry: $retryError');
                    if (mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
              ...
              debugPrint('❌ BookingDocumentManagement: Error checking permission: $permissionError');
              if (mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('Camera permission is required to take photos'),
  ...
  Future<void> _uploadDocument({List<int>? fileBytes, String? fileName}) async {
    ...
    try {
      await _repository.addBookingDocument(
  ...
  Future<void> _deleteDocument(booking_document_model.BookingDocument document) async {
    ...
    try {
      await _repository.deleteBookingDocument(document.id);
  ...
  Widget _buildDocumentCard(booking_document_model.BookingDocument document) {
